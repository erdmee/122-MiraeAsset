...
from ..external.playwright_news_crawler import PlaywrightNewsCrawler
from ..core.news_to_graph import NewsToGraphPipeline
...
class EnhancedDataCollector:
    ...
    def __init__(self):
        ...
        # Graph DB 파이프라인 초기화
        self.news_to_graph = NewsToGraphPipeline()
        ...
        if use_playwright:
            try:
                ...
                if news_items:
                    ...
                    # Graph DB 업데이트 (Playwright 성공)
                    try:
                        print(">>> Playwright 뉴스 → Graph DB 업데이트 시작")
                        update_result = self.news_to_graph.process_latest_news()
                        if update_result and update_result.get("success"):
                            print(
                                f">>> Graph DB 업데이트 성공: {update_result.get('entities_created', 0)}개 엔티티, {update_result.get('relationships_created', 0)}개 관계"
                            )
                        else:
                            print(
                                f">>> Graph DB 업데이트 실패: {update_result.get('error', 'Unknown error')}"
                            )
                    except Exception as e:
                        print(f">>> Graph DB 업데이트 중 오류 발생: {e}")
    ...
    def collect_all_data(
        ...
        use_playwright: bool = True,  # 기본값을 True로 복원
    ) -> Dict:
        """전체 데이터 수집 (동기 버전)"""
        print(
        ...
        if use_playwright:
            try:
                ...
                if news_items:
                    ...
                    # Graph DB 업데이트 (비동기 Playwright)
                    try:
                        print(">>> Playwright 뉴스 → Graph DB 업데이트 시작 (비동기)")
                        update_result = self.news_to_graph.process_latest_news()
                        if update_result and update_result.get("success"):
                            print(
                                f">>> Graph DB 업데이트 성공: {update_result.get('entities_created', 0)}개 엔티티, {update_result.get('relationships_created', 0)}개 관계"
    ...
    def _collect_naver_financial_news_fallback(self, limit: int) -> List[NewsItem]:
        ...
        if items:
            ...
            # Graph DB 업데이트 (BeautifulSoup 폴백)
            try:
                print(">>> BeautifulSoup 뉴스 → Graph DB 업데이트 시작")
                update_result = self.news_to_graph.process_latest_news()
                if update_result and update_result.get("success"):
                    print(
                        f">>> Graph DB 업데이트 성공: {update_result.get('entities_created', 0)}개 엔티티, {update_result.get('relationships_created', 0)}개 관계"
    ...
    def _get_top_companies(self) -> List[Dict[str, str]]:
        """시가총액 상위 기업의 corp_code 조회"""
        try:
            corps = self.dart.corp_codes
            # TODO: 시가총액 정보를 이용해 필터링하는 로직 추가
            return corps[:30]  # 임시로 상위 30개 기업만
        except Exception as e:
        ...
        try:
            ...
            for company in top_companies:
                try:
                    ...
                    for _ in range(8):  # 2년치 = 8분기
                        start_date = current_date - timedelta(days=90)  # 3개월씩
                        ...
                        if not disclosures_df.empty:
                            items = [
                                DisclosureItem(
                                    ...
                                    date=row["rcept_dt"],
                                    type=row["corp_cls"],
                                    importance_score=self._calculate_disclosure_importance(
                                        row
                                    ),
                                )
                            ...
                            print(
                                f">> {company['corp_name']} 공시 {len(items)}건 수집 완료 ({start_date.strftime('%Y%m%d')}~{current_date.strftime('%Y%m%d')})"
                            )

                        current_date = start_date  # 다음 3개월 구간으로

                except Exception as e:
                    print(f">> {company['corp_name']} 공시 수집 실패: {e}")
                    continue

            if all_items:
                self._save_disclosures_to_db(all_items[:limit])
                print(f">> 전체 공시 {len(all_items[:limit])}건 저장 완료")
